...
-- =========================
CREATE TABLE IF NOT EXISTS departments (   -- table des départements
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    name VARCHAR(100) NOT NULL,            -- nom du département
...
-- Table des postes
-- =========================
CREATE TABLE IF NOT EXISTS positions (     -- table des postes
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    name VARCHAR(100) NOT NULL,            -- nom du poste
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- Table des utilisateurs
-- =========================
CREATE TABLE IF NOT EXISTS users (         -- table des utilisateurs
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    ...
    role VARCHAR(50) NOT NULL,             -- rôle utilisateur
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS photos (
    id SERIAL PRIMARY KEY,
    employee_id INT REFERENCES employees(id) ON DELETE CASCADE, -- lien 1–N
    file_path VARCHAR(255) NOT NULL,
    is_main BOOLEAN DEFAULT FALSE, -- permet de marquer la photo principale
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
-- =========================
-- Table des employés (remplacée par la nouvelle version)
-- =========================
-- Table employees (on supprime photo_id)
...
-- =========================
-- Table des informations (salaire, appartenance)
-- =========================
CREATE TABLE IF NOT EXISTS informations (  -- table des informations
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    department_id INT REFERENCES departments(id), -- référence département
    position_id INT REFERENCES positions(id),     -- référence poste
    employee_id INT REFERENCES employees(id),     -- référence employé
    salary DECIMAL(10,2),                  -- salaire
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- =========================
CREATE TABLE IF NOT EXISTS leave_types (   -- table des types de congés
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    name VARCHAR(100) NOT NULL,            -- nom du type de congé
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- Table des congés
-- =========================
CREATE TABLE IF NOT EXISTS leaves (        -- table des congés
    id SERIAL PRIMARY KEY,                 -- identifiant unique
    start_date DATE NOT NULL,              -- date de début
    end_date DATE NOT NULL,                -- date de fin
    leave_type_id INT REFERENCES leave_types(id), -- référence type congé
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- =========================
-- Table des types de permissions
-- =========================
CREATE TABLE IF NOT EXISTS permission_types ( -- table des types de permissions
    id SERIAL PRIMARY KEY,                     -- identifiant unique
    name VARCHAR(100) NOT NULL,                -- nom du type de permission
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- =========================
CREATE TABLE IF NOT EXISTS permissions (   -- table des permissions
    id SERIAL PRIMARY KEY,                  -- identifiant unique
    permission_type_id INT REFERENCES permission_types(id), -- référence type
...
-- =========================
-- Table des demandes (liées aux congés et permissions)
-- =========================
CREATE TABLE IF NOT EXISTS requests (      -- table des demandes
    id SERIAL PRIMARY KEY,                  -- identifiant unique
    leave_id INT REFERENCES leaves(id),     -- référence congé
    permission_id INT REFERENCES permissions(id), -- référence permission
    employee_id INT REFERENCES employees(id),     -- référence employé
    request_date DATE NOT NULL,             -- date de la demande
    reason TEXT,                            -- motif
    quantity INT,                           -- quantité (jours/heures)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
